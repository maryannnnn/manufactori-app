...
/**
 * Applies case-study-categories tables using short dbName `csc`
 * (Postgres 63-char limit for nested block enums/tables).
 */
const sql = `
...
-- Drop unused long-named tables from the first apply attempt, if present.
DROP TABLE IF EXISTS case_study_categories_breadcrumbs CASCADE;
DROP TABLE IF EXISTS case_study_categories CASCADE;
ALTER TABLE payload_locked_documents_rels DROP CONSTRAINT IF EXISTS payload_locked_documents_rels_case_study_categories_fk;
DROP INDEX IF EXISTS payload_locked_documents_rels_case_study_categories_id_idx;
ALTER TABLE payload_locked_documents_rels DROP COLUMN IF EXISTS case_study_categories_id;
...
SELECT '20260825_case_study_categories', 1
WHERE NOT EXISTS (
  SELECT 1 FROM payload_migrations WHERE name = '20260825_case_study_categories'
);
`
...
const apply = async () => {
  ...
  try {
    ...
    console.log('csc (case-study-categories) schema applied')
  } finally {
    client.release()
    await pool.end()
  }
